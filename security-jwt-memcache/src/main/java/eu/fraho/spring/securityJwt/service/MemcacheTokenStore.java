...
public class MemcacheTokenStore implements RefreshTokenStore {
    ...
    public static final String DEFAULT_MEMCACHE_TIMEZONE = "UTC";
    ...
    @Value("${fraho.jwt.refresh.cache.memcache.timezone:" + DEFAULT_MEMCACHE_TIMEZONE + "}")
    private ZoneId cacheTimezone = ZoneId.of(DEFAULT_MEMCACHE_TIMEZONE);
    ...
    public boolean useToken(String username, String deviceId, String token) {
        ...
        final byte[] toCheck;
        try {
            toCheck = token.getBytes(StandardCharsets.UTF_8);
        } catch (IllegalArgumentException iae) {
            throw new JwtRefreshException("Could not decode refresh token", iae);
        }
    ...
    private List<String> listAllKeys(Optional<String> filter) {
        ...
        for (Integer slab : slabs) {
            ...
            if (filter.isPresent()) {
                entries.stream().filter(e -> e.matches(filter.get())).forEach(result::add);
            } else {
    ...
    private Map<String, List<RefreshToken>> listRefreshTokensByPrefix(String filter) {
        ...
        final List<String> keys = listAllKeys(Optional.of(filter));
