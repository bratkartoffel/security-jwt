...
public class LoginRestController implements CookieSupport {
    ...
    // OpenAPI 3.0
    @Operation(description = "Create a new token using the supplied credentials")
    @ApiResponses({
    ...
    // Swagger 2.0
    @io.swagger.annotations.ApiOperation("Create a new token using the supplied credentials")
    @io.swagger.annotations.ApiResponses({
            @io.swagger.annotations.ApiResponse(code = HttpServletResponse.SC_OK, message = "Generated token"),
            @io.swagger.annotations.ApiResponse(code = HttpServletResponse.SC_BAD_REQUEST, message = "Missing a required field in the request"),
            @io.swagger.annotations.ApiResponse(code = HttpServletResponse.SC_UNAUTHORIZED, message = "Either the credentials are wrong or the user has no access to this api"),
    })
