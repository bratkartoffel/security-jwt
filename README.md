...
| cookie.httpOnly            | true                | The cookie will not be accessible by client JavaScript if enabled (highly recommend)|
| cookie.port                | -1                  | When the client requests a resource with an expired token, he will be redirected to the authentication server to automatically obtain new token cookies. If the authentication server don't run on default http(s) port, then you can use this property to set the portnumber. A value of -1 means that no portnumber will be used in the redirect URI.|
| cookie.secure              | true                | The cookie will only be sent over an encrypted (https) connection (recommend)|
| cookie.fallbackRedirectUrl | /                   | After using the refresh token redirect user to his referer or (if unavailable for any reason) to this path. The path may be a full URI.|
| cookie.path                | /auth/refreshCookie | Sets the path for the RestController, defining the endpoint for refresh requests. This endpoint has to be different to fraho.jwt.refresh.path|
