...
* When not using the boot-starter:
  * Use ```eu.fraho.spring.security.hibernate.HibernateTokenStore``` as ```fraho.jwt.refresh.cache-impl``` configuration value
  * Configure your boot application to pick up our entities (add ```@EntityScan(basePackages = {"eu.fraho.spring.security.hibernate"})``` annotation asside your ```@SpringBootApplication```)
